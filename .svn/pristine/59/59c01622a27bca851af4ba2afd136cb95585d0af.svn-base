    ...
    public class NuPortalRequestService : System.Web.Services.WebService
    {
            ...
            int Status)
        {
            ...
            try
            {
                ...
                if (Convert.ToDateTime(CompOffFor).Year == 1753)
                    return SendEmail(dataReader, 1, false);
                else
                    return SendEmail(dataReader, 16, false);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        [WebMethod]
        public bool CancelRequest(int RequestId, int Operation,bool ? Iscompoff)
        {
            SqlConnection con = new SqlConnection();
            try
            {
                ...
                IDataReader dataReader = cmd.ExecuteReader();
                if(Iscompoff==true)
                    return SendEmail(dataReader, 19, false);
                else
        ...
        [WebMethod]
        public bool UpdateRequestStatus(int RequestId, int EmpId, int StatusId, int Operation,bool ? Iscompoff)
        {
            SqlConnection con = new SqlConnection();
            try
            {
                ...
                if (dataReader.FieldCount > 0)
                {
                    ...
                    else if (Operation == 4 && StatusId != 3)
                        return SendEmail(dataReader, 15, false);
                    else if (Operation != 4)
                        return SendEmail(dataReader, StatusId, false);
                    else
